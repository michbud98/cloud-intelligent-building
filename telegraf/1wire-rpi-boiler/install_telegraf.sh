...
install_configuration()
{
	...
	cp get_sensor_data_1wire.py /etc/telegraf/
	RC=$?	
	if [ $RC -ne 0 ]; then
		echo "ERROR: Cannot place python script in telegraf config directory /etc/telegraf"
	else
		echo "SUCCESS: get_sensor_data_1wire.py placed in /etc/telegraf/get_sensor_data_1wire.py"
	fi

	echo "Changing get_sensor_data_1wire.py ownership to pi"
	chown pi /etc/telegraf/get_sensor_data_1wire.py && chgrp pi /etc/telegraf/get_sensor_data_1wire.py
	RC=$?
